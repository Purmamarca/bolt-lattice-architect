...
exports.useSecureGateway = useSecureGateway;
 ...
 */
async function useSecureGateway(targetIp, data) {
    const normalizedIp = normalizeIp(targetIp);
    // 1. IP Validation (Against normalized IP)
    if (!APPROVED_IPS.includes(normalizedIp)) {
        throw new Error(`CFG Policy Violation: IP ${targetIp} (normalized: ${normalizedIp}) is not in the approved whitelist.`);
    }
    // 2. Quantum-Safe Signing (ML-KEM-768)
    const keyPair = await quantum_safe_1.ml_kem.generateKeyPair("ML-KEM-768");
    const encryptedPayload = await quantum_safe_1.ml_kem.encryptData(data, keyPair.publicKey, "ML-KEM-768");
    ...
    if (parts.length === 4) {
        const normalizedParts = parts.map(part => {
            ...
            if (part.startsWith('0') && part.length > 1)
                return parseInt(part, 8).toString();
