...
import {
  ...
  WebhookServer
} from '../src/lib/threat-intelligence';
import { AdversarialTestOrchestrator } from '../tests/adversarial_test';
...
async function runFullOrchestration() {
  ...
  const testOrchestrator = new AdversarialTestOrchestrator();
  ...
  const reportPath = path.join(
    ...
    '..',
    'reports',
    `orchestration_report_${Date.now()}.json`
  );
...
async function runContinuousSync() {
  ...
  const verifier = new VerifierAlpha(DEFAULT_POLICY);
  const policyManager = new DynamicPolicyManager(verifier);
...
export { runFullOrchestration, runContinuousSync, simulateExternalIntegration };
