...
class StressTestRunner {
     ...
     */
    async runTest(scenario, iteration) {
        const startTime = performance.now();
        try {
            const result = await (0, useSecureGateway_1.useSecureGateway)(scenario.ip, {
                cmd: 'stress_test',
                iteration
            });
     ...
     */
    async runBatch(scenarios, iteration) {
    ...
    generateReport(totalTime) {
        ...
        const bypasses = this.results.filter(r => r.expected === 'BLOCK' && r.actual === 'PASS').length;
        // Count false positives (expected PASS but got BLOCK)
        const falsePositives = this.results.filter(r => r.expected === 'PASS' && r.actual === 'BLOCK').length;
        ...
        // Calculate metrics
        const passAt1 = (this.results.filter(r => r.iteration === 1 && r.expected === r.actual).length / TEST_SCENARIOS.length) * 100;
