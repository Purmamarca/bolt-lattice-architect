...
exports.TEST_CONFIG = exports.RedTeamer = exports.AdversarialTestOrchestrator = void 0;
...
class RedTeamer {
    ...
    generateAttackVectors() {
        // Direct IP attacks
        this.attackVectors.push(...this.generateDirectIPAttacks(TEST_CONFIG.scenarios.directIPs));
        // Obfuscation attacks
        this.attackVectors.push(...this.generateObfuscationAttacks(TEST_CONFIG.scenarios.obfuscatedIPs));
        // Malformed attacks
        this.attackVectors.push(...this.generateMalformedAttacks(TEST_CONFIG.scenarios.malformedIPs));
        // Encryption bypass attacks
        this.attackVectors.push(...this.generateEncryptionAttacks(TEST_CONFIG.scenarios.encryptionTests));
        // Complexity attacks
        this.attackVectors.push(...this.generateComplexityAttacks(TEST_CONFIG.scenarios.complexityTests));
    }
    /**
     * Direct IP attacks (approved and unapproved)
     */
    generateDirectIPAttacks(count) {
     ...
     */
    generateObfuscationAttacks(count) {
        const attacks = [];
     ...
     */
    generateMalformedAttacks(count) {
        const attacks = [];
        const malformedIPs = [
     ...
     * Encryption bypass attacks
     */
    generateEncryptionAttacks(count) {
        const attacks = [];
        const algorithms = [
            'ML-KEM-768',
            'ML-KEM-1024',
     ...
     * Complexity attacks
     */
    generateComplexityAttacks(count) {
        const attacks = [];
        for (let i = 0; i < count; i++) {
...
exports.RedTeamer = RedTeamer;
class AdversarialTestOrchestrator {
    constructor() {
        this.redTeamer = new RedTeamer();
        this.architect = new verifier_alpha_1.VerifierAlpha(security_policy_1.DEFAULT_POLICY);
    }
    /**
    ...
    async runFullSuite() {
        ...
        const results = await this.runConcurrentTests(attackVectors, TEST_CONFIG.concurrency);
     ...
     */
    async runConcurrentTests(contexts, concurrency) {
        const results = [];
        for (let i = 0; i < contexts.length; i += concurrency) {
            const batch = contexts.slice(i, i + concurrency);
            const batchResults = await Promise.all(batch.map(ctx => this.architect.verify(ctx)));
...
exports.AdversarialTestOrchestrator = AdversarialTestOrchestrator;
// ============================================================================
// MAIN EXECUTION
// ============================================================================
async function main() {
    ...
    const reportPath = path.join(__dirname, '..', 'reports', `adversarial_test_${Date.now()}.json`);
